...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex flex-row-reverse h-16 items-center justify-between">
        <motion.div
        ...
        >
          <Link href="/" className="text-xl font-bold">
            Personal Profile
            {mounted && theme === "dark" && <span className="text-xs text-primary mr-2">Dark Mode</span>}
            {mounted && theme === "light" && <span className="text-xs text-primary mr-2">Light Mode</span>}
          </Link>
        </motion.div>
